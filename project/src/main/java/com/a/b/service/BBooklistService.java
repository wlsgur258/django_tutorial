...
public class BBooklistService implements CommonService {
	...
	public void execute(Model model) {
		Dao dao = new Dao();
		ArrayList<Book> books = dao.booklist();

		model.addAttribute("booklist", books);
		
	}

	public List<Book> selectList(int a, int z) throws SQLException{
		Dao dao = new Dao();
		ArrayList<Book> books = dao.booklist();
	...
	public MessageListView getMessageListView(int pageNumber) throws SQLException{
		...
		return new MessageListView(null, messageList1, null, messageTotalCount,currentPageNumber,MESSAGE_COUNT_PER_PAGE,firstRow,endRow);
	}
