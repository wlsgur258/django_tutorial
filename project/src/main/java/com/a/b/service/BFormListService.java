...
public class BFormListService implements CommonService {
	...
	public void execute(Model model) {
		Dao dao = new Dao();
		ArrayList<Customer> members = dao.formList();
		
		model.addAttribute("formList", members);
	}

	public List<Customer> selectList(int a, int z) throws SQLException{
		Dao dao = new Dao();
		ArrayList<Customer> members = dao.formList();
	...
	public MessageListView getMessageListView(int pageNumber) throws SQLException{
		...
		List<Customer> messageList2 = null;
		int firstRow =0;
		int endRow =0;
		...
		return new MessageListView(null, null, messageList2, messageTotalCount,currentPageNumber,MESSAGE_COUNT_PER_PAGE,firstRow,endRow);
	}
